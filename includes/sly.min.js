...
u&&u.eq(g.activeItem).removeClass(a.activeClass);S.empty();C||(D.unbind("."+l),p.add(t).css(v||(a.horizontal?"left":"top"),v?"none":0),h.removeData(f,l));va=0;return c};c.init=function(){if(!va){c.on(na);if(!C){}
!v&&"static"===E.css("position")&&E.css("position","relative");Z();z?(ja(a.startAt),c[K?"toCenter":"toStart"](a.startAt)):
