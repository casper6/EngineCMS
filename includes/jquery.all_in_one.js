...
function sho(pid,name,act,id_razdel,cid,edit_pole,clean_url) {
	if (clean_urls == 0) re_link = '-'+name+'_page_'+pid;
	else re_link = clean_url;
	...
	if (document.getElementById('pid'+pid).innerHTML=='') {
		$(".pid").html(""); 
		document.getElementById('pid'+pid).innerHTML = ' <ul class="button-bar"><li class="first"><a href="'+re_link+'" target="_blank" title="Открыть страницу на сайте">'+icon('blue small','s')+'</a></li><li><a target=_blank href="sys.php?op=base_pages_edit_page&pid='+pid+'#1" title="Редактировать страницу">'+icon('orange small','7')+'</a></li><li class="punkt"><a onclick=replace("'+pid+'") title="Копировать/Переместить/Создать ярлык">'+icon('blue small','^')+'</a></li><li class="punkt"><a onclick=offpage('+pid+',0) title="'+active+'</li><li class="last punkt"><a onclick=delpage("'+pid+'") title="Удалить страницу">'+icon('red small','F')+'</a></li></ul>'+add_pole;
	}
...
function papka_show(cid, name, sort, id, xxx, relink) {
	show('podpapka'+cid);
	if (clean_urls == 0) re_link = '-'+name+'_cat_'+cid;
	else re_link = '/'+name+'/'+relink+'/';
	if (document.getElementById('papka'+cid).innerHTML=='') { 
		document.getElementById('papka'+cid).innerHTML = '<br><ul class="button-bar"><li class="first"><a target="_blank" href="'+re_link+'" title="Открыть эту папку на сайте">'+icon('blue small','s')+' Открыть</a></li><li><a target="_blank" href="sys.php?op=edit_base_pages_category&cid='+cid+'#1" title="Редактировать папку">'+icon('orange small','7')+' Редактировать</a></li><li class="last"><a onclick=delpapka("'+cid+'") style="cursor:pointer;" title="Удалить папку">'+icon('red small','F')+'</a></li></ul>';
...
function save_spisok() {
    ...
    $.ajax({
	  ...
	  beforeSend: function(){ $('#add').html('Сохраняю...'); },
      success: function(data) { $('#add').hide('slow'); 
...
function translite(str){
	var arr={'а':'a', 'б':'b', 'в':'v', 'г':'g', 'д':'d', 'е':'e', 'ж':'g', 'з':'z', 'и':'i', 'й':'y', 'к':'k', 'л':'l', 'м':'m', 'н':'n', 'о':'o', 'п':'p', 'р':'r', 'с':'s', 'т':'t', 'у':'u', 'ф':'f', 'ы':'y', 'э':'e', 'А':'A', 'Б':'B', 'В':'V', 'Г':'G', 'Д':'D', 'Е':'E', 'Ж':'G', 'З':'Z', 'И':'I', 'Й':'Y', 'К':'K', 'Л':'L', 'М':'M', 'Н':'N', 'О':'O', 'П':'P', 'Р':'R', 'С':'S', 'Т':'T', 'У':'U', 'Ф':'F', 'Ы':'I', 'Э':'E', 'ё':'yo', 'х':'h', 'ц':'ts', 'ч':'ch', 'ш':'sh', 'щ':'shch', 'ъ':'!', 'ь':'!', 'ю':'yu', 'я':'ya', 'Ё':'YO', 'Х':'H', 'Ц':'TS', 'Ч':'CH', 'Ш':'SH', 'Щ':'SHCH', 'Ъ':'!', 'Ь':'!',
	'Ю':'YU', 'Я':'YA'};
	var replacer=function(a){return arr[a]||a};
	str = str.replace(/[\s\S]/g,replacer);
	return str.replace('!',''); // А-яёЁ
}
...
(function($) {
    ...
    $.widget('ui.tagit', {
        ...
        _cleanedInput: function() {
            ...
            return $.trim(this.tagInput.val().replace(/^"(.*)"$/, '$1').replace(',', ''));
        },
