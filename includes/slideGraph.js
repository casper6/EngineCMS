var currentSection = "pane2-pane"; // The default loaded section on the page
var tabTag = "-tab";
var paneTag = "-pane";

// Scroll the page manually to the position of element "link", passed to us.

function ScrollSection(link, scrollArea, offset, navDiv) {
	
	if(navDiv) {
		var insObj = document.getElementById(navDiv);
		var tds = document.getElementsByTagName('TD');
		for(var no=0; no<tds.length; no++) {
			if(tds[no].className == 'itemsBarActive' ) {
				tds[no].className = 'itemsBar';		
			}
		}
		insObj.className = 'itemsBarActive';
	}
...
var scrollanim = {time:00, begin:0, change:00, duration:0.0, element:null, timer:null};
