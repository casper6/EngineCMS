...
class Upload
{
	...
	var $fileSize = 0;
	var $validImageExts = array("gif", "jpg", "jpeg", "png");
	var $errors = array();
	...
	function isPermittedFileExt($validFileExt = array())
	{
		...
		if(sizeof($validFileExt) && sizeof($this->invalidFileExt))
		{
			...
			{
				if(array_search($ext, $this->invalidFileExt) !== false)
				{
		...
		if(sizeof($validFileExt))
		{
			if(array_search($this->getFileExt(), $validFileExt) !== false)
			{
				return true;
			}
		}elseif(array_search($this->getFileExt(), $this->invalidFileExt) === false)
		{
			return true;
		}
	...
	function isImage($invalidImageExts = array(), $delete = true)
	{
		...
		$ValidImageExts = $this->validImageExts;
		if(sizeof($invalidImageExts))
		{
			foreach ($ValidImageExts as $v)
			{
				if(array_search($v, $invalidImageExts) !== false)
				{
					$ValidImageExts[] = $v;
				}
			}
	...
	/**
		 ...
		 * @return boolean
		 */
	function _imageSave($newImageHandler, $fileName, $quality = 80)
	{
