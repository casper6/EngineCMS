...
<ol class='li_light'>
<li><b>Логотип панели администрирования</b> является ссылкой на эту страницу, также как и следующая за ним кнопка «Содержание». Вы всегда сможете вернуться на эту страницу, нажав по ним.</li>
<li onmouseover=\"$('#site_button span.icon').addClass('polosa_red');\" onmouseout=\"$('#site_button span.icon').removeClass('polosa_red');\"><b>Кнопка «На сайт»</b> открывает главную страницу вашего сайта.</li>
<li onmouseover=\"$('#search_button span.icon').addClass('polosa_red');\" onmouseout=\"$('#search_button span.icon').removeClass('polosa_red');\"><b>Поиск по сайту</b> — даже если на вашем сайте не установлен поиск (а сделать это можно очень быстро) — в Панели администрирования поиск работает аналогично — пишите поисковое слово или словосочетание, жмете Enter и поиск выдает результаты по найденному на страницах, а также в папках и разделах сайта.</li>
<li onmouseover=\"$('ul.button-bar').addClass('polosa_red');\" onmouseout=\"$('ul.button-bar').removeClass('polosa_red');\"><b>Меню администрирования</b> — содержит 4 основных раздела. Содержание — открыт в данный момент — позволяет управлять содержимым сайта — его разделами, папками, страницами, комментариями и блоками. Также содержит текстовое поле для заметок и напоминаний — «Записки администратора».</li>
<li onmouseover=\"$('#logout_button span.icon').addClass('polosa_red');\" onmouseout=\"$('#logout_button span.icon').removeClass('polosa_red');\"><b>Выход из администрирования</b> — эту кнопку стоит нажимать после окончания всех работ по администрированию сайта только, если вы редактируете сайт за чужим компьютером или в интернет-кафе.</li>
<li onmouseover=\"$('#razdel_td').addClass('polosa_red');\" onmouseout=\"$('#razdel_td').removeClass('polosa_red');\"><b>Разделы сайта</b> — вся левая колонка посвящена созданным для сайта разделам, которые, в свою очередь, могут содержать папки и страницы или сами являться отдельными страницами. По сторонам от заголовка «Разделы:» находятся две кнопки — для сортировки (↓) и создания новых разделов (+). Колонка с разделами может скрываться при отображении не относящейся к ним информации на этом месте (белое поле). У созданного раздела после его названия может быть кнопка редактирования главной (и единственной) страницы раздела или кнопка добавления страниц (если раздел может содержать страницы).
</li>
<li onmouseover=\"$('.polosa_razdelitel').addClass('polosa_red');\" onmouseout=\"$('.polosa_razdelitel').removeClass('polosa_red');\"><b>Полоса сворачивания левой панели</b> служит для скрытия/раскрытия колонки разделов.
<li onmouseover=\"$('#soderganie_menu').addClass('polosa_red');\" onmouseout=\"$('#soderganie_menu').removeClass('polosa_red');\"><b>Панель быстрого доступа</b> к основным функциям редактора сайта. 
...
<p><a class='h2 punkt' onclick=\"$('#better_change_text').toggle('slow');\">Работа с текстом в редакторе</a>.
<div id=better_change_text style='display:none;'>".close_button('better_change_text')."
<p>1. Чтобы страницы в разделе выглядели как заголовки без текста — вставляем текст не в Предисловие, а в Содержание.
<p>2. Чтобы справиться с изменением шрифта при вставке из Ворда или со страницы сайта — после того, как вставили текст, выделяем весь текст ( нажать Ctrl + A ), нажимаем в редакторе кнопку B (жирный) один или два раза, пока весь текст не станет жирным, затем жмем еще 1 раз, чтобы весь стал нормальным, затем выбираем в меню редактора Параграф (вторая кнопка) пункт «Обычный текст».
<p>3. Как делать отступы. Там, где нужно отступить, жмем не Enter, а Shift + Enter нужное количество раз (чаще хватает одного раза) - такой отступ никуда не денется. Также можно поставить разделительную полосу - в редакторе есть для этого кнопка. Делая отступ, ставим курсор не в конец предложения, после которого отступ, а в начало предложения, перед которым будет отступ! Это важно.
<p>4. Проблема при вставке текста — тире исчезают. Проблема с исчезновением тире после копирования и вставки текста из Adobe Illustrator, Microsoft Word и PageMaker. Иногда вместо тире эти программы используют альтернативный символ, который исчезает после копирования и вставки в обычное текстовое поле Блокнота или поле любого редактора на любом сайте (не зависит от его системы управления). Для того, чтобы решить проблему, необходимо в той программе, откуда копируем (например Ворде), выделить один из таких символов «тире», скопировать его, открыть окно Поиска и Замены, вставить в поле поиска (вставится пустое место, но символ можно будет найти), а в поле замены поставить обычное тире и пройтись по всему тексту. После этого текст можно копировать.
</div>
