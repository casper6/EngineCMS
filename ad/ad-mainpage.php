...
<script>function add_raz() { if ($('#razdels').val() != '') $('#add_razdel').val( $('#add_razdel').val() + $('#razdels').val() + ',' ); }</script>
	...
	<td>Режим меню: <a class='punkt' href=\"javascript:$('#re_menu').toggle('slow');\">Справка</a>.
      <div id=re_menu style='display:none;'>".close_button('re_menu')."Меню сайта может настраиваться автоматически или вручную. Если выбран авто-режим — при редактировании разделов, папок и страниц можно будет выбрать, в каком автоматическом меню их показывать и на каком порядковом месте. В ручном режиме меню создается по простым правилам (для того, чтобы быть универсальным и легко переключать варианты отображения):<br>[элемент открыть][url=/]Главная[/url][элемент закрыть]<br>[элемент открыть][url=#]Пункт меню 1[/url][элемент закрыть]<br>[элемент открыть][url=#]Пункт меню 2[/url]<br>&nbsp;&nbsp;[уровень открыть]<br>&nbsp;&nbsp;[элемент открыть][url=#]Подпункт 1[/url][элемент закрыть]<br>&nbsp;&nbsp;[элемент открыть][url=#]Подпункт 2[/url][элемент закрыть]<br>&nbsp;&nbsp;[уровень закрыть]<br>[элемент закрыть]<br><i>где # - это ссылка на страницу.</i><br>В меню может быть до 3-х уровней вложенности</div>
	</td>
	...
	if ($name != 31) redactor($red, $text, 'text'); // редактор: типа редактора, редактируемое поле
		...
		if ($name != 10) codemirror("javascript", "text");
	}
