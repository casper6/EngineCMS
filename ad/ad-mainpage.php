...
function edit_main($id) {
	...
	if ($type == "5") { ############################### ОТКРЫТИЕ БАЗА ДАННЫХ
		...
		<p>Параметры разделяются знаком &, который нельзя использовать в значениях параметров.

		<h2>Параметр type=</h2> Доступность на сайте (1 — доступен для вывода на сайте, 2 — доступен только администратору).
		<h2>Параметр status=</h2> Позволяет изменить стандартный список статусов, записывается как «Название|цвет», разделяется «#!#». По-умолчанию: Удалено|#fcccd7#!#Утверждено|#dddddd#!#Проверить|#eeeeee#!#В работе|#ffffff
		<h2>Параметр message=</h2> Сообщение справа при добавлении информации в базу данных, можно использовать HTML. По-умолчанию пусто.
		<h2>Параметр del_stroka=</h2> Показывать кнопку удаления строки: по-умолчанию 1 - показывать, 0 - не показывать. 
		<h2>Параметр edit_stroka=</h2> Показывать кнопку редактирования строки: по-умолчанию 1 - показывать, 0 - не показывать.
		<h2>Параметр num_day_stroka=</h2> Значение|Поле с датой|Ограничение. Максимальное количество строк, добавляемых за 1 день, по-умолчанию 0 - без ограничений. Ограничение = false (нельзя добавить эту дату) или true (дата выделена красным). Пример: 50|data|true
		<h2>Параметр num_day_sum_stroka=</h2> Значение|Поле с датой|Поле с числом|Ограничение. Максимальная сумма значений числовых строк, добавляемых за 1 день, по-умолчанию 0 - без ограничений. Ограничение = false (нельзя добавить эту дату) или true (дата выделена красным). Пример: 50|data|chislo|false
		<h2>Параметр lim=</h2> Количество выводимых строк, по-умолчанию - 100.

		<h2>Параметр options=</h2> Содержит поля и их настройки<br>
		Разделитель полей: /!/ <br>
		Разделитель настроек полей: #!# <br>
		Формат полей: Англ название#!#Рус название#!#Тип данных#!#Важно#!#Видимость#!#Замена#!#Фильтр <br>
		...
		Замена: шаблон для первоначального значения текстового или строкового поля<br>
		Фильтр: 1 - включен, 0 - выключен. Позволяет добавлять фильтр по этому полю в окно поиска по базе данных.<br>";
		...
		if ($type==5) { //////////////////////////////////////////////////////////
			...
			for ($x=0; $x < $n; $x++) {
				...
				switch ($one[2]) {
					...
					case "число": $sql2[] = "`$one[0]` VARCHAR( 1000 ) NOT NULL"; break;
