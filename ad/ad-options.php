...
<pre>&lt;a onclick=\"shop_add_tovar([page_id],[price],1)\"&gt;Купить&lt;/a&gt;
&lt;button onclick=\"shop_add_tovar([page_id],[price],1)\"&gt;Купить&lt;/button&gt;</pre>
	
Вместо блоков можно использовать и цифры, например (2 - номер страницы товара, 590 — стоимость в рублях, 1 - количество):<br>
<pre>&lt;a onclick=\"shop_add_tovar(2,'590',1)\"&gt;Купить&lt;/a&gt;</pre>
...
onclick=\"shop_add_tovar([page_id],'[price]',1);\"&gt;В Корзину&lt;/a&gt;&lt;/div&gt;
&lt;/div&gt;</pre>
...
<p>Осуществляется с помощью поля ввода или выбора цифрового значения. Если у вас маловероятно закажут больше одного товара (к примеру, крупногабаритная или дорогая розница) — вряд ли понадобится делать выбор количества. Если обычно не заказывают больше определенного небольшого количества, например 10 — можно сделать поле для выбора из заранее подготовленных значений, от 1 до 10. Если количество может быть гораздо больше и ни от чего не зависит — рациональнее всего сделать поле для текстового ввода количества. Подстановка цифровых значений из этих полей происходит за счет JavaScript или jQuery. Например так: shop_add_tovar([page_id],'[price]',$('#count').val()); и добавим перед кнопкой «Купить» &lt;input type='number' size='5' id='count' value='1' class='page_shop_count'&gt; Естественно такой вариант подойдет только, если на странице только один товар, например для страницы самого товара. Если так сделать в разделе, в списке страниц — получится несколько элементов с одинаковым id, поэтому используем: 
<pre>&lt;input type='number' min='0' id='count_[page_id]' value='1' class='page_shop_count'&gt;</pre> и <pre>shop_add_tovar([page_id],'[price]',$('#count_[page_id]').val());</pre><br>
