...
// pid module cid title open_text main_text date counter active golos comm foto search mainpage price --- pages
// cid module title description pic sort counter parent_id --- category
...
if (!in_array($name, $module_names) and $cid == 0 and $pid == 0) {

$sql = "SELECT p.cid pccid, pcom.cid cid, pcom.num num, pcom.avtor avtor, pcom.mail mail, pcom.text text, pcom.ip ip, pcom.data data, pcom.golos golos, pcom.adres adres, pcom.tel tel, pcom.active active FROM ".$prefix."_pages_comments pcom, ".$prefix."_pages p WHERE pcom.num=p.pid order by data desc";

}

if (in_array($name, $module_names)) {
$sql = "SELECT p.cid pccid, pcom.cid cid, pcom.num num, pcom.avtor avtor, pcom.mail mail, pcom.text text, pcom.ip ip, pcom.data data, pcom.golos golos, pcom.adres adres, pcom.tel tel, pcom.active active FROM ".$prefix."_pages_comments pcom, ".$prefix."_pages p WHERE pcom.num=p.pid and p.module='".$name."' order by data desc";
...
//$name = $papka_module[$cid];
$razdel = "<a href=/sys.php?op=base_comments&name=".$name.">".$module_title[$name]."</a> | <a href=/sys.php?op=base_comments>Открыть комментарии всех разделов</a>";
}
...
<br>Комментарии лучше и быстрее всего можно изменить с помощью HTML-режима <img align=bottom src=images/sys/edit_0.png title=\"Изменить в HTML\">.<br>
Розовым цветом отображаются выключенные или непроверенные администратором комментарии.</div>
...
while ($rows = $db->sql_fetchrow($result)) {
    ...
    $comm_ip = $rows['ip'];
    /////////////////////////////
    $comm_mail = $rows['mail'];     if (trim($comm_mail) != "") $comm_mail = ",<br><b>E-mail:</b> ".$comm_mail;
    $comm_adres = $rows['adres'];   if (trim($comm_adres) != "") $comm_adres = ",<br><b>Адрес:</b> ".$comm_adres;
    $comm_tel = $rows['tel'];       if (trim($comm_tel) != "") $comm_tel = ",<br><b>Телефон:</b> ".$comm_tel;
    ////////////////////////////
    $act = $rows['active'];
    ...
    <a href=/sys.php?op=base_comments&name=".$raz." title='Открыть комментарии этого раздела'><b>".$module_title[$raz]."</b></a> &rarr;&nbsp;<a href=/sys.php?op=base_comments&name=".$raz."&cid=".$pc_cid." title='Открыть комментарии этой папки'>".$papka_title[$pc_cid]."</a></td><td><b>Название стр.:</b> <a href=/sys.php?op=base_comments&name=".$raz."&pid=".$comm_page." title='Открыть комментарии этой страницы'>".$page_title[$comm_page]."</a> <a target=_blank href=/-".$raz."_page_".$comm_page."#comment_".$comm_cid." title='Открыть комментарий на сайте'><img src=images/sys/magnify.png align=bottom><b style='color: #1191bb;'>Открыть на сайте</b></a><br><b>Комментарий №".$comm_cid.":</b> ".$comm_text."</td><td><nobr><a href=/sys.php?op=base_comments_edit_comments&cid=".$comm_cid."><img src=images/sys/edit_1.png title=\"Изменить\"></a> <a href=/sys.php?op=base_comments_edit_comments&cid=".$comm_cid."&red=1><img src=images/sys/edit_0.png title=\"Изменить в HTML\"></a> <a href=/sys.php?op=base_comments_status&name=".$raz."&pid=".$comm_page."&cid=".$comm_cid."><img class='icon2 i43' src='/images/1.gif' align=right title='Отключить/Включить комментарий'></a> &nbsp; &nbsp; <a href=/sys.php?op=base_pages_delit_comm&cid=".$comm_cid."&ok=ok&pid=".$comm_page."><img class=\"icon2 i21\" src=/images/1.gif title=\"Удалить\"></a></nobr>
    </td></tr>";
...
global $tip, $admintip, $prefix, $db, $red; //, $redaktor, $toolbars;
...
if ($comm_act==1) $check= " checked"; else $check= " unchecked";
...
echo "<script type=\"text/javascript\" src=\"ed/js/redactor/redactor.js\"></script> 
<link rel=\"stylesheet\" href=\"ed/js/redactor/css/redactor.css\" type=\"text/css\" media=\"screen, projection\" /> 
<script type=\"text/javascript\"> 
$(document).ready(function()
{  $('#comm_text').redactor({ focus: true, css: ['/css_20'], upload: 'upload.php' }); });
</script>
<textarea id=\"comm_text\" name=\"text\" rows=\"15\" cols=\"80\" style='width:100%;'>".$comm_text."</textarea><br>";
}
...
function comm_links($records,$r_start=0,$URL,$inpage=20) {
...
function base_comments_status($name, $pid, $cid) {
    ...
    $act = $row['active'];
    if ($act == 0 or $act == 2) {
    $db->sql_query("UPDATE ".$prefix."_".$tip."_comments SET active='1' WHERE cid='$cid'") or die('Не удалось включить комментарий. Вероятно какие-то проблемы с базой данных. Администратор поможет решить проблему.');
    }
    if ($act == 1) {
    $db->sql_query("UPDATE ".$prefix."_".$tip."_comments SET active='0' WHERE cid='$cid'") or die('Не удалось отключить комментарий. Вероятно какие-то проблемы с базой данных. Администратор поможет решить проблему.');
    }
