...
if ($func == "opengarbage") { // Открытие вкладок Содержания
  ...
  if ($id == 8) { // ПОМОЩЬ
    $pageslistdel .= "<p>Ознакомьтесь с основными элементами. Наведите мышкой на элементы нижеприведенного списка (красный круг покажет на этот элемент):</p>
    <img id=krug src='images/krug.gif' style='position: fixed; z-index:1000; left: 2000px; top: 2000px;' onmouseover=\"$(this).css({left: '2000px', top: '2000px'});\">
<ol class='li_light'>
...
<li onmouseover=\"$('#krug').css({left: '800px', top: '5px'});\"><b>Поиск по сайту</b> — даже если на вашем сайте не установлен поиск (а сделать это можно очень быстро) — в Панели администрирования поиск работает аналогично — пишите поисковое слово или словосочетание, жмете Enter и поиск выдает результаты по найденному на страницах, а также в папках и разделах сайта.</li>
...
<li onmouseover=\"$('#krug').css({left: '200px', top: '65px'});\"><b>Разделы сайта</b> — вся левая колонка посвящена созданным для сайта разделам, которые, в свою очередь, могут содержать папки и страницы или сами являться отдельными страницами. По сторонам от заголовка «Разделы:» находятся две кнопки — для сортировки (↓) и создания новых разделов (+). Колонка с разделами может скрываться при отображении не относящейся к ним информации на этом месте (белое поле).<a href=# onclick=\"$('#about').show('slow');\">Узнать больше о разделах</a>.
      <div id=about style='display:none;'>".close_button('about')."
...
<ul>
  ...
  <li>Кнопка «Корзина» — содержит удаленные страницы, которые можно отредактировать, удалить окончательно или восстановить на прежнем месте (при наличии их раздела). При отсутствии удаленных страниц Корзина не видна.
  <li>Кнопка «Старое» — это резервные копии страниц с возможностью замены текущей страницы на одну из её прошлых версий.
  <li>Кнопка «Блоки» — показывает список небольших элементов сайта — блоков.
  <li>Кнопка «+ страницу» — открывает редактор для добавления страницы
</ul>
</li>
</ol>
...
/*
Добавление блока:
вкладка «Оформление» — кнопка «+» — Добавить блок.
В открывшемся окне сразу пишем название блока (можно по-русски), желательно без предлогов. Выбираем его тип, например «Текст или HTML» и в появившемся поле «Содержание блока:» прописываем его содержание, т.е. тот текст, который будет показывать наш блок.
Жмем кнопку «Сохранить» в левом верхнем углу.
Перед нами окно настройки блока.
Если мы поставили ему адекватное название, которое можно вывести как заголовок над его содержанием — выбираем в поле «Заголовок блока» — «показывать».
Также может понадобиться поле «Показывать блок только в определенном разделе». Если блок нужно показывать в каком-то определенном разделе, у которого нет своего отдельного дизайна, можно выбрать в этом поле нужный раздел.

Остается поставить блок. Обычно блоки ставятся в Дизайн. Главный дизайн сайта или дизайн, созданный для определенного раздела. Открываем дизайн на редактирование: Оформление — Дизайн разделов и блоков — нужный нам дизайн — белый карандаш (редактирование). Ищем место, в которое нужно поставить. Если вы не разбираетесь в HTML – ищите по другим уже поставленным блокам или обратитесь к разработчику. Ставим название блока в квадратных скобках. Например: [блок]

Отключить блок можно двумя способами — убрать его название в квадратных скобках из дизайна или просто временно отключить на странице блоков (Оформление — Блоки).

Если в тексте нужно поставить ссылку на одну из статей сайта, это можно сделать из редактора и без знания HTML – открываем созданный блок на редактирование (белый карандаш), жмем справа сверху кнопку «Редактор» и выбираем один из редакторов. Пишем текст ссылки, выделяем его, жмем кнопку с изображением цепи «Ссылка» — Вставить ссылку — в поле URL пишем адрес страницы, на которую нужно перейти после нажатия на ссылку — Вставить. Сохраняем блок (слева сверху кнопка «Сохранить»).
*/
  } elseif ($id == 5) { // НОВОЕ
