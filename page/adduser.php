...
$row = $db->sql_fetchrow($result); $catspisok = substr($row['text'],0,-1);
$soderganie .= '<div id="list_razdel">Выберите раздел<br>';
...
if ($_POST["submit"]=="Добавить") {
  if(!$_POST["title"] || !$_POST["open_text"] || !$_POST["main_text"]) {
    $soderganie .="<p class='errormes'>Вы ввели не все данные, попробуйте еще раз</p>";
  } else {
  ...
  $sql = "select pid from ".$prefix."_pages where title='".$title."' and date='".$data."'"; $result = $db->sql_query($sql); $row = $db->sql_fetchrow($result);
  $page_id = $row['pid'];
    // РАБОТА СО СПИСКАМИ
  ...
  foreach ($_POST["add"] as $name => $elements) {
    ...
    $result = $db->sql_query($sql); $row = $db->sql_fetchrow($result); $s_id = $row['id'];
...
$insert = array(); 
foreach ($period as $per) { if (!in_array($per, $noupd)) $insert[] = "(NULL, '".$name.", '".$per."', '', '0', ' $page_id ', '0')"; }
 $insert = implode(", ",$insert);
$sql = "SELECT name, pages FROM ".$prefix."_spiski WHERE type='".$name."' and (".$update.") order by name"; $result = $db->sql_query($sql);
 while ($row = $db->sql_fetchrow($result)) {
$na = $row['name']; // дата
$pa = $row['pages']; // страницы
}
if (trim($insert) != "") { $db->sql_query("INSERT INTO ".$prefix."_spiski (id, type, name, opis, sort, pages, parent) VALUES ".$insert.";") or die ('Ошибка: Не удалось сохранить списки. 5');
}
 break;
...
}$soderganie .= '<p class="errormes">Материал добавленн!</p><br>'; recash('/--users_'.$_COOKIE['user_id']);}
if($_GET["id"] != "0"){
// получаем список доспных разделов для добавления
$soderganie .= "<form class=regforma' action='--adduser_".$_COOKIE['user_group']."_".$id."' method='post' enctype='multipart/form-data'>Выберите категорию:<select name='categor'>";
$result = $db->sql_query("select title, cid from ".$prefix."_pages_categories where `module`='$id'"); while ($row = $db->sql_fetchrow($result)) 
{$soderganie .= '<option value=\"'.$row['cid'].'\">'.$row['title'].'</option>';}
	$soderganie .= '</select><br>';
	$soderganie .= "<h2>Название (заголовок)</h2><textarea class=big name=title rows=1 cols=10 style='font-size:16pt; width:100%;'></textarea><br>
	 <h2>Краткое содержание</h2><textarea cols=80 id=editor name=open_text rows=10></textarea><br>
	 <h2>Основной текст</h2><textarea cols=80 id=edit name=main_text rows=15></textarea><br>";
	   $sql = "select text from ".$prefix."_mainpage where name='$id' and type='2'";
  $result = $db->sql_query($sql);
  $row = $db->sql_fetchrow($result);
  $tex = $row['text'];
  // это галерея?
  if (strpos($tex,"view=5")) $soderganie .= "<b>Фото (для фотогалереи):</b> <input type=file name=foto size=40><br>
  ...
  // это магазин?
  if (strpos($tex,"view=3")) $soderganie .= "<b>Стоимость:</b> <input type=text name=price size=3 value='0'> руб.<br>";
  else $soderganie .= "<input type=hidden name=price value=''>";
// Подсоединие списков
$result = $db->sql_query("select id from ".$prefix."_mainpage where `name`='$id'"); $row = $db->sql_fetchrow($result); $cat = $row['id'];
  ...
  case "3": // период времени
    $soderganie .="<br><br><b>".$s_title.":</b> (выберите даты из меню, кликнув по значкам)<br>
  ...
  case "2": // файл file=pic&papka=/img=verh&resizepic=x&file=&picsize=600&minipic=1&resizeminipic=x&minipicsize=100
    switch($fil) {
      case "pic": $type_fil = "картинка"; break; case "doc": $type_fil = "документ/архив"; break; case "flash": $type_fil = "flash-анимация"; break; case "avi": $type_fil = "видео-ролик"; break;
    }
    switch($mesto) {
      case "verh": $type_mesto = "сверху"; break; case "niz": $type_mesto = "снизу"; break;
    }
    $type_mini="";
  ...
  $soderganie .="<br><br><b>$s_title:</b><br>";
             ...
             $soderganie .="<select size=10 multiple=multiple name='add[$s_name][]' style='font-size:11px;'><option value=0> не выбрано </option>";
  ...
  $soderganie .= '<br><input type="submit" name="submit" value="Добавить"></form>';
}
