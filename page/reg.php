...
if (validate_email($email)) { 
	...
	if ($result2 && $db->sql_fetchrow($result2) > 0) {
		$soderganie .= 'ОШИБКА - адрес электронной почты уже существует.';
	} else {
		...
		if (!$result) {
			$soderganie .= 'ОШИБКА - Ошибка базы данных.'; 
		} else {
	...
	$soderganie .= 'ОШИБКА: email введен неправильно.';
}
