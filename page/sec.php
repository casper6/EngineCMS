...
##########################################################################################
  ...
  if (stripos($agent,"DISCo Pump") || stripos($agent,"Offline Explorer") || stripos($agent,"Teleport") || stripos($agent,"WebZIP") || stripos($agent,"WebCopier") || stripos($agent,"Wget") || stripos($agent,"FlashGet") || stripos($agent,"CIS TE") || stripos($agent,"DTS Agent") || stripos($agent,"WebReaper") || stripos($agent,"HTTrack") || stripos($agent,"Web Downloader")) { 
    //if ($ban = true) banned($_SERVER["REMOTE_ADDR"]);
    die(ss("Использование программ для автоматического скачивания запрещено!")); 
  }
  if ($agent==" ") {
    //if ($ban = true) banned($_SERVER["REMOTE_ADDR"]);
    die(ss("Программист, это - такой человек, который сначала много думает, чтобы потом ничего не делать!")); 
  }
  
  unset($pagetitle);
  if(!defined('END_TRANSACTION')) { // удалить?
  ...
  if (stripos($postString,'%20union%20') OR stripos($postString,'*/union/*') OR stripos($postString,' union ') OR stripos($postString_64,'%20union%20') OR stripos($postString_64,'*/union/*') OR stripos($postString_64,' union ') OR stripos($postString_64,'+union+') OR stripos($postString_64,'http://') OR (stripos($postString_64,'cmd=') AND stripos($postString_64,'&cmd')===false) OR (stripos($postString_64,'exec') AND stripos($postString_64,'execu')===false) OR stripos($postString_64,'concat') OR (stripos($postString,'http-equiv')) OR (stripos($postString_64,'http-equiv')) OR (stripos($postString_64,'alert(')) OR (stripos($postString,'javascript:')) OR (stripos($postString_64,'javascript:')) OR (stripos($postString,'document.cookie')) OR (stripos($postString_64,'document.cookie')) OR (stripos($postString,'onmouseover=')) OR (stripos($postString_64,'onmouseover=')) OR (stripos($postString,'document.location')) OR (stripos($postString_64,'document.location'))) {
    die(ss("Попытка взлома, тип 4<br>Возможно, что вы ввели «javascript:» перед JS-запросом в ссылке. Делать этого не нужно, т.к. и без этого JS-код будет работать."));
  }
  ...
  $htmltags = ss("Вы использовали запрещенные символы HTML-кода. Вероятно вы - взломщик. Нажмите в браузере «Назад».");
