...
$soderganie .= "<form class=regforma' action='--edituser_".$_COOKIE['user_id']."' method='post'>
		...
		<br><input type='submit' name='submit' value='Обновить имя'></form>";
if ($_POST["submit"] == "Обновить имя") {
  if (!$_POST["user_name"]) {
    $soderganie .= "<p class='errormes'>Вы не ввели свое имя</p>";
  } else {
    $user_name = $_POST["user_name"];
    $db->sql_query("UPDATE ".$prefix."_users SET `name`='$user_name' WHERE `user_id`='".$_COOKIE['user_id']."' ;"); 
    user_set_tokens($_COOKIE['email']);
    $soderganie .= '<p class="errormes">Имя обновленно, дождитесь обновления страницы <meta http-equiv="Refresh" content="6"/>';
  }
}
...
<br><input type='submit' name='submit2' value='Обновить фото'></form>";
if ($_POST["submit2"] == "Обновить фото") { 
  if (!$_FILES["file"]["size"]) {
    $soderganie .= "<p class='errormes'>Вы не выбрали фотографию</p>";
  } else {
    // удаляем старое фото
    ...
    if($_FILES["file"]["size"] > 5242880) { // 1024*5*1024
      $soderganie .= "<p class='errormes'>Размер файла превышает пять мегабайт</p>";
    } else {
      ...
      if(is_uploaded_file($_FILES["file"]["tmp_name"])) {
        ...
        if($_FILES["file"]['type'] == "image/gif" || 
          ...
          user_set_tokens($_COOKIE['email']);
          $soderganie .= "Фото обновленно, дождитесь обновления страницы";
          $soderganie .= '<meta http-equiv="Refresh" content="6"/>';
        } else {
            $soderganie .= "<p class='errormes'>Можно загружать только изображения в форматах jpg, jpeg, gif и png.";
        }
      } else {
...
if ($numrows > 0) {
      ...
      if ($name != "") {
        ...
        switch($type) {
          ...
          case "4":
            ...
            if ($numrows2 == 1) {
            $db->sql_query("UPDATE ".$prefix."_spiski SET `name`='$elements' WHERE `type`='$name' and pages='1,".$_COOKIE['user_id']."';") or die ('Ошибка: Не удалось сохранить список. 8');
            } else  {
            $db->sql_query("INSERT INTO ".$prefix."_spiski (id, type, name, opis, sort, pages, parent) VALUES (NULL, '".$name."', '".$elements."', '', '0', '1,".$_COOKIE['user_id']."', '0');") or die ('Ошибка: Не удалось сохранить список. 3');
            } 
            break;
          ////////////////////////////////////////////////////////////////////////////
          ...
          case "1":
            ...
            if ($numrows2 == 1) $db->sql_query("UPDATE ".$prefix."_spiski SET `name`='$elements' WHERE `type`='$name' and pages='1,".$_COOKIE['user_id']."';") or die ('Ошибка: Не удалось сохранить список. 8');
            else $db->sql_query("INSERT INTO ".$prefix."_spiski (id, type, name, opis, sort, pages, parent) VALUES (NULL, '".$name."', '".$elements."', '', '0', '1,".$_COOKIE['user_id']."', '0');") or die ('Ошибка: Не удалось сохранить список. 3');
          break;
          ////////////////////////////////////////////////////////////////////////////
          case "0": // список
      ...
      case "2": // файл file=pic&papka=/img=verh&resizepic=x&file=&picsize=600&minipic=1&resizeminipic=x&minipicsize=100
        switch($fil) {
          case "pic": 
            ...
            break; 
          case "doc": 
            $type_fil = "документ/архив"; 
            break; 
          case "flash": 
            $type_fil = "flash-анимация"; 
            break; 
          case "avi": 
            $type_fil = "видео-ролик"; 
            break;
        }
        $type_mini="";
  ...
  $soderganie .= '<br><input type="submit" name="submit2" value="Обновить"></form>';
} // end if
