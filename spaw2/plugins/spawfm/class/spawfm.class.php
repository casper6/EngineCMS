...
class SpawFm
{
  ...
  function getFileThumbnail($file, $filename) // <============================ ???
  {
    //$icons = SpawFm::getFileIconsDefinition($filename);
    //return '../plugins/spawfm/img/'.$icons['icon_big'];
    
    $img = '../plugins/spawfm/img/list.gif';
    
    $file = explode("spaw2/",$file);
$file = "/spaw2/".$file[1]; // base64_encode(

if (strpos($file,".")) {

$tip = explode(".", $file);
$last = count($tip)-1;
$tip = strtolower($tip[$last]);
if ($tip=="jpeg" or $tip=="jpg" or $tip=="png" or $tip=="gif") {
$img = '../plugins/spawfm/img/ico_image.gif';
...
//$imageQuality='60';
//$Zoom_Crop='1';
//$img = '/includes/phpThumb/phpThumb.php?src='.$file .'&w='. $imageWidth .'&h='. $imageHeight . $Zoom_Crop .'&q='.$imageQuality.'';
}
if ($tip=="doc") $img = '../plugins/spawfm/img/doc.gif';
if ($tip=="xls") $img = '../plugins/spawfm/img/xls.gif';
if ($tip=="pdf") $img = '../plugins/spawfm/img/pdf.gif';
if ($tip=="mp3" or $tip=="wma" or $tip=="wav") $img = '../plugins/spawfm/img/mp3.gif';
if ($tip=="zip" or $tip=="tg" or $tip=="gz" or $tip=="rar" or $tip=="7z") $img = '../plugins/spawfm/img/zip.gif';
if ($tip=="txt") $img = '../plugins/spawfm/img/ico_default.gif';
if ($tip=="swf") $img = '../plugins/spawfm/img/ico_flash.gif';
} else {
//$icons = SpawFm::getFileIconsDefinition($filename);
$img = '../plugins/spawfm/img/ico_folder.gif';
}
    return $img; //'../../resize.php?type=2&f='.$file.'&name='.$filename;
  }
  ...
  function getFilesList()
  {
    ...
    foreach ($files as $key=>$file) {
      ...
      $files[$key] = array(
        ...
        'thumb'     => $this->getFileThumbnail($this->getCurrentFsDir().$file, $file),
        'other'     => $other
      );
    }
