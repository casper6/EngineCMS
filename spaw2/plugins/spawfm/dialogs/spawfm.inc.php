  ...
  <td colspan="2" style="border-bottom: 1px solid #000000;">
    ...
    <tr>
      <td valign="middle" width="400">
        <select name="dir" style="width: 400px;" onchange="document.spawfm_form.submit()">
          <?php
          foreach ($dirs as $dir) {
      ...
      <td valign="middle">
        ...
        if (!empty($curr_dir_subdir)) {
          echo '<input type="image" onclick="SpawFm.goUpClick();" src="../plugins/spawfm/img/btn_up.gif" title="'.$lang->m('go_up', 'buttons').'" class="bt" style="width: 24px; height: 24px; margin: 1px;" /> <a href=# onclick="SpawFm.goUpClick();">Вверх</a> ';
        } else {
          echo '<input type="image" src="../plugins/spawfm/img/btn_up_off.gif" title="'.$lang->m('go_up', 'buttons').'" class="bt" style="width: 24px; height: 24px; margin: 1px;" disabled="disabled" /> Вверх ';
        }
        if (!$spawfm->getCurrentDirSetting('recursive') or 
            !$spawfm->getCurrentDirSetting('allow_create_subdirectories')) 
        {
          echo '<input type="image" src="../plugins/spawfm/img/btn_new_folder_off.gif" title="'.$lang->m('create_directory', 'buttons').'" class="bt" style="width: 24px; height: 24px; margin: 1px;" disabled="disabled" /> Создать папку ';
        } else {
          echo '<input type="image" onclick="SpawFm.createDirectoryClick();" src="../plugins/spawfm/img/btn_new_folder.gif" title="'.$lang->m('create_directory', 'buttons').'" class="bt" style="width: 24px; height: 24px; margin: 1px;" /> <a href=# onclick="SpawFm.createDirectoryClick();">Создать папку</a>';
        }
        ?>
      </td>
      <td valign="middle" align="right">
        ...
        &nbsp;
        <select name="type" style="width: 180px;" onchange="document.spawfm_form.submit()">
...
<tr>
  <td colspan="2" style="border-bottom: 1px solid #000000;">
    <input type="button" style="background-color: red; color: white;" name="delete_button" value="<?php echo $lang->m('delete', 'buttons')?>" class="bt" disabled="disabled" onclick="SpawFm.deleteClick()" />
    <input type="button" style="background-color: skyblue; color: blue;" name="rename_button" value="<?php echo $lang->m('rename', 'buttons')?>" class="bt" disabled="disabled" onclick="SpawFm.renameClick()" />
    <?php    
    ...
    ?>
    &nbsp;
    <input type="file" name="upload_file" class="input" style="background-color: purple; color: white;" />
    <input type="submit" style="background-color: purple; color: white;" name="upload_button" value="<?php echo $lang->m('upload', 'buttons')?>"<?php echo $st; ?> class="bt" />
  </td>
</tr>
<tr>
  <td align="left" valign="top" style="padding: 5px;">
    <div id="error"><?php echo $error_msg; ?></div>
    <b><font color=red>Имена файлов и папок должны быть на английском, без пробелов!</font><br>Поиск по имени файла — навести на картинку и искать по алфавиту.</b>
  </td>
  <td align="right" valign="bottom" style="padding: 5px;">
  <?php 
  $url = getenv("REQUEST_URI"); 

  if ($url=="" or strpos($url, "scid")) echo "
  <input type=\"button\" style=\"background-color: green; color: white;\" name=\"ok_button\" value=\"".$lang->m('ok', 'buttons')."\" onClick=\"SpawFm.okClick()\" class=\"bt\" />
  <input type=\"button\" style=\"background-color: gray; color: white;\" name=\"cancel_button\" value=\"".$lang->m('cancel', 'buttons')."\" onClick=\"SpawFm.cancelClick()\" class=\"bt\" />"; 
  else echo "<a href=/admin>Вернуться назад</a>
  
  <input type=\"hidden\" name=\"ok_button\" value=\"".$lang->m('ok', 'buttons')."\" onClick=\"SpawFm.okClick()\" class=\"bt\" disabled=\"disabled\" />
  <input type=\"hidden\" name=\"cancel_button\" value=\"".$lang->m('cancel', 'buttons')."\" onClick=\"SpawFm.cancelClick()\" class=\"bt\" />"; 
  ?>
