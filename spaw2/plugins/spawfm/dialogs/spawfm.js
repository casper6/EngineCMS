...
SpawFm.allowModify = true;
SpawFm.allowModifySubdirectories = true;
SpawFm.txtFileSize = 'Size';
SpawFm.txtFileDate = 'Date modified';
SpawFm.txtNoFileSelected = 'Выберите файл, а потом уже нажимайте ГОТОВО!';
SpawFm.txtConfirmDelete = 'Вы реально хотите удалить файл [*file*]?';
SpawFm.txtRename = 'Введите имя нового файла "[*FILE*]":';
SpawFm.txtCreateDirectory = 'Введите имя для новой папки:';
SpawFm.txtConfirmDeleteDir = 'Вы уверены, что хотите удалить папку "[*DIR*]"?';
SpawFm.txtDownload = '[download file]';
SpawFm.previewImgObj = true;
SpawFm.pathSeparator = '|';
SpawFm.onloadSelectFile = true;
...
SpawFm.listItems = function() {
  ...
  for (var i=0; i<SpawFm.items.length; i++) {
    ...
    var newBlock = doc.createElement('span');
    newBlock.className = 'fm_file';
    
    if (item['type'] == 'D') newBlock.setAttribute('style', 'display:block;');
    else newBlock.setAttribute('style', 'display:inline;');
    
    newBlock.setAttribute('id', 'item_'+i);
    if (item['type'] == 'D') newBlock.setAttribute('style', 'display:block;');
    // thumb
    var divBlock = doc.createElement('span');
    divBlock.className = 'fm_file_thumb';
    divBlock.setAttribute('style', 'display:inline;');
    divBlock.setAttribute('style', 'width:50;');
    
        //if (item['type'] == 'D') newBlock.setAttribute('style', 'display:block;');
          //and itemdata['descr']!=".jpg" and itemdata['descr']!=".gif" and itemdata['descr']!=".png"
    var img = doc.createElement('img');
    img.setAttribute('src', item['thumb']);
    img.setAttribute('title', item['name']);
    img.setAttribute('hspace', '3');
    
    if (item['type'] != 'D') img.setAttribute('style', 'height:50;width:50;');
    //img.setAttribute('border', '1');
    x = SpawFm.filePath + item['name'];
    if (item['type'] != 'D') img.setAttribute('onmouseover', 'this.src="/includes/phpThumb/phpThumb.php?src='+x+'&w=50&h=50&q=0"');
    /*
    
    ../../resize.php?type=2&f='+x+'&name='+item['name']+'
    
    $imageWidth='40';
$imageHeight='40';
$imageQuality='60';
$Zoom_Crop='1';
$img = '/includes/phpThumb/phpThumb.php?src='.item['thumb'].'&w=60&h=40&q=60
*/
    ...
    */
    ...
    // title
    var divTitle = doc.createElement('inline');
    //divBlock.setAttribute('style', 'color:red;');
    //divTitle.setAttribute('style', 'color:red;');
    //divTitle.setAttribute('style', 'display:none;');
    divTitle.className = 'fm_file_title';
    //if (item['type'] != 'D') divTitle.setAttribute('style', 'display:none;');
    divTitle.setAttribute('id', 'file_title_'+i);
    //divTitle.setAttribute('onmouseover', 'this.src="/includes/phpThumb/phpThumb.php?src='+x+'&w=80&h=80&q=40"');
    if (item['type'] == 'D') var txt = doc.createTextNode("Папка "+item['name']); // ВАЖНО!!!
    if (item['type'] == 'D') divTitle.setAttribute('style', 'cursor:pointer; text-decoration:underline;');
    if (item['type'] == 'D') divTitle.appendChild(txt); // ВАЖНО!!!
