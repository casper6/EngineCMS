...
function typo($html, $lang = 'ru')
{
	...
	$html = preg_replace('/В·/i', "&bull;", $html);
	$html = preg_replace('/вЂў/i', "&bull;", $html);

	$html = preg_replace('/В«/i', "&laquo;", $html);
	$html = preg_replace('/В»/i', "&raquo;", $html);

	$html = preg_replace('/\.{3}/i', "&hellip;", $html);
	$html = preg_replace('/\((c|СЃ)\)/i', "&copy;", $html);

	$html = preg_replace('/\(r\)/i', '<sup><small>&reg;</small></sup>', $html);
	$html = preg_replace('/\(tm\)/i', '<sup><small>&trade;</small></sup>', $html);
	$html = preg_replace('/(\d+)(x|С…)(\d+)/i', '$1&times;$3', $html);
